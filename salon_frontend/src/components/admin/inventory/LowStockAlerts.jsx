...
export default function LowStockAlerts() {
    return (
        <div className="flex flex-col h-full slide-right overflow-hidden bg-surface/10">
            {/* Header / Banner */}
            <div className="p-8 bg-rose-50 dark:bg-rose-950/20 border-b border-rose-100 dark:border-rose-950/40 flex items-center justify-between">
                ...
                <div className="hidden md:flex gap-3">
                    <button className="px-4 py-2 bg-surface border border-rose-200 dark:border-rose-900 rounded-xl text-xs font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-all shadow-sm">
                        Mute for 24h
                    </button>
                    <button className="px-4 py-2 bg-rose-600 rounded-xl text-xs font-bold text-primary-foreground hover:shadow-lg hover:shadow-rose-600/30 transition-all">
                        Create Bulk Order
                    </button>
                </div>
            </div>
...
function AlertCard({ alert }) {
    ...
    return (
        <div className={`p-6 bg-surface border-2 rounded-3xl transition-all hover:shadow-xl hover:-translate-y-1 group ${isCritical ? 'border-rose-100 dark:border-rose-900/50' : 'border-orange-100 dark:border-orange-900/50'}`}>
            <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-3">
                    <div className={`p-2.5 rounded-xl ${isCritical ? 'bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400' : 'bg-orange-50 dark:bg-orange-950/30 text-orange-600 dark:text-orange-400'}`}>
                ...
                <div className={`px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-widest border ${isCritical ? 'bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 border-rose-100 dark:border-rose-900/50' : 'bg-orange-50 dark:bg-orange-900/10 text-orange-600 dark:text-orange-400 border-orange-100 dark:border-orange-900/30'}`}>
                    {alert.status}
                </div>
            </div>
