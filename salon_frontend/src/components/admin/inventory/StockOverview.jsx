...
export default function StockOverview() {
    const { products, outlets, productCategories } = useInventory();
        ...
        <div className="flex flex-col h-full slide-right overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="relative">
                        ...
                        <select 
                            ...
                            className="w-full pl-10 pr-4 py-2.5 bg-surface-alt border border-border rounded-xl text-sm font-semibold text-text-secondary appearance-none focus:outline-none focus:ring-2 focus:ring-primary/10 transition-all uppercase tracking-wider cursor-pointer"
                        >
                    ...
                    <div className="relative">
                        ...
                        <select 
                            ...
                            className="w-full pl-10 pr-4 py-2.5 bg-surface-alt border border-border rounded-xl text-sm font-semibold text-text-secondary appearance-none focus:outline-none focus:ring-2 focus:ring-primary/10 transition-all uppercase tracking-wider cursor-pointer font-bold"
                        >
                ...
                <table className="w-full text-left border-collapse min-w-[1000px]">
                    <thead className="sticky top-0 z-10">
                        <tr className="bg-surface border-b border-border">
                            ...
                            <th className="px-6 py-4 text-[10px] font-bold text-text-secondary uppercase tracking-widest bg-surface">Assigned Salon</th>
                    ...
                    <tbody className="divide-y divide-border bg-surface text-sm">
                        {filteredStock.map((item) => (
                            <tr key={item.id} className="hover:bg-surface/50 transition-colors group cursor-default">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[10px] text-text-muted font-bold uppercase tracking-wider">SKU: {item.sku}</span>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-4">
                                ...
                                <td className="px-6 py-4">
                                    <span className="text-text-secondary font-medium text-xs">{item.outlet || 'Central'}</span>
                                </td>
                                <td className="px-6 py-4">
                                    <span className={`text-sm font-bold ${item.status === 'Low Stock' || item.status === 'Critical' ? 'text-rose-600' : 'text-text'}`}>
                                        {item.stock} {item.unit || 'Units'}
                                    </span>
                                </td>
                                <td className="px-6 py-4">
                                    <span className="text-xs font-semibold text-text-muted">{item.threshold} {item.unit || 'Units'}</span>
                                </td>
                                <td className="px-6 py-4">
            ...
            <div className="p-4 border-t border-border bg-surface/30">
                <div className="flex items-center justify-between px-4 text-[10px] font-bold text-text-muted uppercase tracking-widest">
                    <span>Showing {filteredStock.length} of {products.length} active stock lines</span>
