        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-surface p-6 rounded-[2rem] border border-border/40 relative overflow-hidden">
                    <p className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] mb-4">Inflow History</p>
                    <h3 className="text-3xl font-black text-emerald-500">₹{totalAdded.toLocaleString()}</h3>
                    <p className="text-[10px] text-text-muted font-bold mt-2 uppercase italic tracking-widest">Lifetime Fund Injections</p>
                </div>

                <div className="bg-surface p-6 rounded-[2rem] border border-border/40 relative overflow-hidden">
                    <p className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] mb-4">Outflow Burn Rate</p>
                    <h3 className="text-3xl font-black text-rose-500">₹{totalSpent.toLocaleString()}</h3>
                    <p className="text-[10px] text-text-muted font-bold mt-2 uppercase italic tracking-widest">Total Expenses Logged</p>
                </div>
            </div>
                            ...
                            <tbody className="divide-y divide-border/10">
                                {filteredTransactions.map((txn) => (
                                    <tr key={txn.id} className="group hover:bg-primary/5 transition-colors">
                                        ...
                                        <td className="py-5">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${txn.type === 'FUND_ADDED' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'}`}>
                                                    {txn.type === 'FUND_ADDED' ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
                                                </div>
                                                <div>
                                        ...
                                        <td className="py-5 text-right">
                                            <span className="text-[10px] font-black text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-md uppercase tracking-tighter">Verified</span>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
