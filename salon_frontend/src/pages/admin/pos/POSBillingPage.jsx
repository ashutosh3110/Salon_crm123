                    ...
                    <div className="bg-surface rounded-none border border-border p-6 space-y-4 shadow-sm">
                        ...
                        {showNewClient && (
                            <form onSubmit={handleCreateClient} className="bg-surface-alt rounded-none border border-border p-5 space-y-4 animate-in slide-in-from-top-4 duration-300">
                                ...
                                <input required value={newClientForm.name} onChange={e => setNewClientForm(p => ({ ...p, name: e.target.value.replace(/[^a-zA-Z\\s]/g, '') }))} placeholder="Full Entity Name *" className="w-full px-4 py-3 rounded-none border border-border bg-surface text-[10px] font-black uppercase tracking-[0.2em] focus:ring-2 focus:ring-primary/20 outline-none" />
