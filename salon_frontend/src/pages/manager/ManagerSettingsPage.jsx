...
export default function ManagerSettingsPage() {
    return (
        <div className="max-w-3xl mx-auto space-y-8">
            ...
            {/* Profile Brief */}
            <div className="bg-gradient-to-br from-primary to-primary-dark rounded-3xl p-6 text-white flex flex-col sm:flex-row items-center gap-6 shadow-xl shadow-primary/20 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full blur-3xl -translate-y-8 translate-x-8" />
                <div className="relative w-20 h-20 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center text-3xl font-black border border-white/20 shadow-lg">
                    AS
                </div>
                <div className="flex-1 text-center sm:text-left relative">
                    ...
                    <div className="flex flex-wrap justify-center sm:justify-start gap-4 mt-4">
                        <div className="bg-black/20 backdrop-blur-sm px-3 py-1 rounded-lg border border-white/10 text-[10px] font-black uppercase tracking-widest">Store #42</div>
                        <div className="bg-black/20 backdrop-blur-sm px-3 py-1 rounded-lg border border-white/10 text-[10px] font-black uppercase tracking-widest">Premium Member</div>
                    </div>
                </div>
                <button className="relative px-5 py-2.5 bg-white text-primary rounded-xl text-xs font-black uppercase tracking-widest shadow-lg hover:scale-105 active:scale-95 transition-all">
                    View Public Profile
                </button>
            </div>
            ...
            <div className="space-y-6">
                {settingsSections.map((section) => (
                    <div key={section.title} className="space-y-3">
                        ...
                        <div className="bg-surface rounded-3xl border border-border/40 overflow-hidden shadow-sm divide-y divide-border/40">
                            {section.items.map((item) => (
                                <button key={item.id} className="w-full flex items-center justify-between p-4 sm:p-5 hover:bg-surface-alt transition-colors text-left group">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-xl bg-background flex items-center justify-center border border-border/10 group-hover:scale-110 transition-transform">
                                            <item.icon className="w-4.5 h-4.5 text-text-secondary group-hover:text-primary transition-colors" />
                                        </div>
                                        <div>
            ...
            {/* Logout */}
            <div className="pt-4">
                <button className="w-full bg-rose-500/5 hover:bg-rose-500/10 border border-rose-500/10 text-rose-500 rounded-2xl p-4 flex items-center justify-center gap-2 text-sm font-black uppercase tracking-widest transition-all hover:scale-[1.01] active:scale-[0.99]">
